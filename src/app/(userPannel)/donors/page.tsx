...
const Donors = () => {
  ...
  //   const processedDonors = [];
  //   const userEmail = "admin@bloodsathi.com"; // Required by Nominatim to prevent bans
  ...
  //     // ==========================================
  //     // 2. SLOW PATH: Needs geocoding via API
  //     // ==========================================
  //     setGeocodingProgress(`Locating ${donor.full_name || 'donor'} on map (${i + 1}/${data.length})...`);
  ...
  //       // --- ATTEMPT 2: The PIN Code Trick ---
  //       if (!lat) {
  //         const pinMatch = addressText.match(/\b\d{6}\b/);
  //         if (pinMatch) {
  //           const pinCode = pinMatch[0];
  //           res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(pinCode + ", India")}&email=${userEmail}`);
  //           geoData = await res.json();
  ...
  //         res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(cleanedAddress)}&email=${userEmail}`);
  //         geoData = await res.json();
  ...
  //       // ==========================================
  //       // 3. SAVE COORDS TO SUPABASE FOR NEXT TIME
  //       // ==========================================
  //       if (lat && lng) {
  //         await supabase
  //           .from("register")
  ...
  //       // Wait 1.5 seconds between API calls to prevent getting banned
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        if (!lat) {
          const lower = addressText.toLowerCase();
          let fallback = "";
          if (lower.includes("kolkata") || lower.includes("salt lake")) fallback = "Kolkata, West Bengal, India";
          else if (lower.includes("habra")) fallback = "Habra, West Bengal, India";
          else if (lower.includes("barasat")) fallback = "Barasat, West Bengal, India";
